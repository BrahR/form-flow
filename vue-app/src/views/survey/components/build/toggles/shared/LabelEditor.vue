...
const useQuestionFactory = inject(USE_QUESTION_FACTORY);
const useQuestionToggles = useQuestionTogglesStore();
const { label } = useQuestionToggles;
</script>

<template>
  <div class="sharedBuild_toggle_input_row_wrapper sharedBuild_ltr">
    <div class="questionDescription_description_Wrapper">
      <div>
        <div class="toggleButton_wrapper">
          <span class="toggleButton_title toggleButton_bold">Label</span>
          <label v-if="label.toggleable" class="toggleButton_switch">
            <input
              v-model="label.on"
    ...
    <div v-show="label.on" class="ck-show-toolbar-wrapper mt-3">
      <!--   @vue-ignore goofy ahh ahh ts  -->
      <ckeditor
        :editor="label.editor"
        :config="label.config"
        @ready="label.ready"
      ...
      <InputError :error="useQuestionFactory?.store?.labelError" />
    </div>
  </div>
</template>
...
.questionDescription_description_Wrapper {
  width: 100%;
  display: flex;

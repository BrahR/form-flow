...
const { statementButton } = storeToRefs(useQuestionToggles);

watch(
  () => statementButton.value.value, // i *cri*
  (value) => {
    if (!value) {
      statementButton.value.error = true;
      return;
    }
    statementButton.value.error = false;
  },
);
</script>

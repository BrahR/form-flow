...
watch(
  () => welcomeButton.value.value, // i *cri*
  (value) => {
    console.log(value);
    if (!value) {
      welcomeButton.value.error = true;
      return;
    }
...
<template>
  <div class="sharedBuild_title_row_wrapper sharedBuild_ltr">
    ...
    <InputError :show="welcomeButton.error" error="Button value is required." />
  </div>
</template>

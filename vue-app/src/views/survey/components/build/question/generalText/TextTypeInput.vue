...
const validate = () => {
  ...
  schema.value
    .validate(value)
    .then(() => {
      textAnswerFormat.value.error.text = false;
    })
    .catch((_err) => {
      textAnswerFormat.value.error.text = true;
    });
};
...
watch(
  ...
  () => {
    if (textAnswerFormat.value.selected.type !== "text") return;
...
<template>
  <span class="textQuestion_hotkey_wrapper">
    <input
      ...
      :class="{
        textQuestion_hasError: textAnswerFormat.error.text,
      }"
  ...
  <div
    v-if="textAnswerFormat.error.text"
    class="textQuestion_continue_button_wrapper"
    :class="{
      textQuestion_text_question_error: textAnswerFormat.error.text,
    }"
  >
